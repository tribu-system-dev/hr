...
def post_init_hook(env):
    """Migrate calendars from hr.version (formerly hr.contract) to
    calendar_ids to have consistent work schedule history"""
    ...
    for employee in employees.filtered("version_ids"):
        ...
        for version in employee.version_ids.sorted("date_start"):
            if version.resource_calendar_id == employee.resource_calendar_id:
                continue
            ...
            cal_ids = employee.calendar_ids.filtered(
                lambda x, version=version: x.calendar_id
        ...
        # set correct calendar in the current version
        employee.version_id.resource_calendar_id = employee.resource_calendar_id
